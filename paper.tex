...
This construction performs phase gradients using $4n + O(1)$ T gates, which is apparently independent of $\epsilon$.
However, there are in fact three ways in which the construction's cost does depend on $\epsilon$.
...
We covered a few examples of optimizing T-counts with our constructions but, for brevity, we leave further low hanging fruit as an exercise for the reader.
...
Regardless of whether there will be further T-count improvements for operations as basic as Toffoli gates appearing in pairs, it is clear that many other T-count optimizations are waiting to be found.
