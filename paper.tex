...
We discuss some specific examples in \autoref{sec:other-applications} (e.g. applying the same rotation $R_Z(\theta)$ to several qubits).
...
In \autoref{sec:other-applications}, we discuss several circuit constructions and estimates improved by our adder and temporary logical-AND constructions.
...
For example, consider a register being conditionally incremented several times.
Incrementing is performed with paired Toffoli gates that can be replaced by temporary logical-ANDs.
An alternative way to reduce the T-count is by cancelling computation work at the start of an increment against uncomputation work from the end of the previous increment.
The alternative approach happens to have the same T-count in this case, but that may not always be the case.
...
Regardless of whether there will be further T-count improvements for operations as basic as the temporary logical-AND, it is clear that many other T-count optimizations are waiting to be found.
