...
\begin{figure}
  ...
  \caption{
	Adder circuit building-block with a marginal T-count of 4.
  }
  \label{fig:full-adder-block}
\end{figure}

Our adder circuits use temporary logical-AND operations.
We define the temporary-AND computation (which we draw as a wire emerging from two controls) in \autoref{fig:compute-logical-AND}.
We define the corresponding uncomputation (which we draw as a wire merging into two controls) in \autoref{fig:erase-logical-AND}.
Computing the temporary-AND has a T-count of 4, but when uncomputing a temporary-AND we have access to measurement only need Clifford operations.
...
\begin{figure}
  ...
  \caption{
	...
	The $|A\rangle$ state input on the bottom wire contributes to the T-count (because $|A\rangle$ states are the resource used to perform T gates).
  }
